...
import picture from "./picJs/picture.js"
import crookedPicture from "./picJs/crookedPicture.js"
import spookyBoi from "./picJs/spookyBoi.js"
import doorKnobPic from "./picJs/doorKnob.js"
...
import { step, ticking, playDoor, clownLaugh, doorHandleJiggle } from "./audio.js"
...
myFont.load().then(function (font) {
    ...
    function animate() {
        ...
        if ((player.position.x > (100 - scrollOffset)) && (player.position.x < (420 - scrollOffset)) && lastKey == 'interact') {
            topText = "Jammed"
            botText = " "
        } else if ((player.position.x > (930 - scrollOffset)) && (player.position.x < (1245 - scrollOffset)) && doorKnob == false && lastKey == 'interact') {
            topText = "Hm, it's missing the handle. I can't get in"
            botText = " "
        } else if ((player.position.x > (1390 - scrollOffset)) && (player.position.x < (1700 - scrollOffset)) && doorKnob == true && lastKey == 'interact') {
            topText = "The missing doorknob!"
            botText = "This might come in handy..."
        //else if you pick up door knob, "great I have this knob"
        } else {
            topText = " "
            botText = " "
        }

        if ((player.position.x > (1860 - scrollOffset)) && (player.position.x < (2260 - scrollOffset)) && lastKey == 'interact') {
            topText = "Darn, it's locked"
            botText = " "
        }
        
        if ((player.position.x > (930 - scrollOffset)) && (player.position.x < (1245 - scrollOffset)) && onDoor == true) {
            topText = "That fits perfectly!"
            botText = "I wonder where this door leads..."
        }


        if (crooked == true && spooked == false) {
            genericObjects.push(new GenericObject({
                x: 0,
                y: 0,

                image: createImage(spookyBoi)
            }))
            genericObjects.forEach((genericObject) => {
                genericObject.draw()
            })
    ...
    window.addEventListener('keydown', ({ keyCode }) => {
        switch (keyCode) {
            ...
            case 69:
                ...
                if ((player.position.x > (1390 - scrollOffset)) && (player.position.x < (1700 - scrollOffset)) && crooked == false) {
                    clownLaugh()
                    genericObjects[1].image = createImage(crookedPicture)
                    genericObjects[1].position.y = genericObjects[1].position.y - 50
                    genericObjects.push(new GenericObject({
                        x: 1541 - scrollOffset,
                        y: 830,
                        image: createImage(doorKnobPic)
                    }))
                    crooked = true
                }
                if ((player.position.x > (1390 - scrollOffset)) && (player.position.x < (1700 - scrollOffset)) && doorKnob == false && spooked == true) {
                    doorKnob = true
                    genericObjects.pop()
                }
                //if door knob is on door location to bathroom
                if ((player.position.x > (922 - scrollOffset)) && (player.position.x < (1272 - scrollOffset)) && onDoor == false && doorKnob == true) {
                    onDoor = true;
                    genericObjects.push(new GenericObject({
                        x: 1268 - scrollOffset,
                        y: 556,
                ...
                } else if ((player.position.x > (922 - scrollOffset)) && (player.position.x < (1272 - scrollOffset)) && onDoor == true) {
                    playDoor()
                    setTimeout(function () {
